...
class Hr extends CI_Controller {
	...
	***************************************************************************************************************************************/
    ...
    public function login_check(){
		...
		$session = $this->session->userdata('login'); //GETS SESSIONS DETAILS

    	if ($session['logged_in'] != '1') //CHECKS IF LOGGED IN
			{
				redirect("https://{$_SERVER['HTTP_HOST']}/w1279154/index.php/authentication"); //IF NOT LOGGED IN REDIRECTS TO LOGIN
			}
			else {
				echo('<div align="right">Logged in as '.$session['username'].', <a class="button_logout" id="auth" onClick="logout()">Logout</a></div>');
			}	//ELSE DIPLAYS LOGIN INFO ON TOP OF PAGE
    }
	...
	***************************************************************************************************************************************/
	...
	{
		$datestring = "%Y-%m-%d"; //SETS DATE STRING FORMAT
		$time = time(); //GETS TODAYS TIME
		$date = mdate($datestring, $time); //SETS TODAYS DATE IN FORMAT
	...
	***************************************************************************************************************************************/
	...
	{
		$this->load->view('default_head'); //LOADS DEFAULT HEAD
		$this->load->view('hr_menu'); //LOADS HR MENU
	...
	***************************************************************************************************************************************/
	public function remove_employee_search()
	{
		$data['search'] = 1;
		$this->load->view('default_head'); //LOADS DEFAULT HEAD
		$this->load->view('remove_employee_search', $data); //LOADS REMOVE EMP SEARCH
	...
	***************************************************************************************************************************************/
	...
	{
		$data['emp_no'] = $this->input->post('emp_no');  //GETS EMP NUMBER

		$this->set($data['emp_no']); //GETS EMP NUMBER TO GLOBAL
		...
		{
			$data['is_manager'] = "TRUE"; //SETS MANAGER INFO
		}
